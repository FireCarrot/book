...
def reload_schedule(driver, wait=0.1):
    ...
    if check_alert_present:
        driver.execute_script(simulate_click, driver.find_element_by_id("btnReloadSchedule"))
    try:
        _ = WebDriverWait(driver, 10).until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, "div#loadingLayer"))
        )
    except:
        pass
...
def find_and_select_seats(driver, count, wait):
    while (count == 0):
        ...
        try:
            ...
            print("loading layer was not gone in 10 seconds so it will be refreshed now")
            return reload_schedule(driver)
